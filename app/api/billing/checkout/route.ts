...
import { getStripe, getStripePriceId, type BillablePlanId } from '@/lib/stripe'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const billablePlanId = planId as BillablePlanId
    ...
    // 買い切りプランは再購入可能なので、already-active チェックをスキップ
    if (
      billablePlanId !== 'one-time' &&
    ...
    const isOneTime = billablePlanId === 'one-time'
