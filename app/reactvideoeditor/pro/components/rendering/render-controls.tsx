  ...
  return (
    <>
      <Popover onOpenChange={() => setHasNewRender(false)}>
        ...
        <PopoverContent className="w-60 p-3">
          <div className="space-y-1.5">
            <h4 className="text-sm font-extralight">最近の書き出し</h4>
            {renders.length === 0 ? (
              <p className="text-xs text-muted-foreground">まだ書き出し履歴はありません</p>
            ) : (
              renders.map((render) => (
                <div
                  key={render.id}
      ...
      <Button
        ...
        title={!isRenderingEnabled ? "書き出しは現在無効です" : undefined}
      >
        {!isRenderingEnabled ? (
          "動画を書き出す"
          ...
          <>
            ...
            書き出し中... {(state.progress * 100).toFixed(0)}%
          </>
