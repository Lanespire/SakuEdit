...
export const useTimelineHandlers = ({
  ...
  const pendingOverlaysRef = React.useRef<Overlay[] | null>(null);
  ...
  const handleTracksChange = React.useCallback((newTracks: TimelineTrack[]) => {
    ...
    pendingOverlaysRef.current = transformTracksToOverlays(newTracks);
    ...
    flushTimeoutRef.current = window.setTimeout(() => {
      if (pendingOverlaysRef.current) {
        setOverlays(pendingOverlaysRef.current);
