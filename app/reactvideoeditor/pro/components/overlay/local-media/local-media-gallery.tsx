  ...
  const handleFileUpload = useCallback(async (
    ...
    if (files && files.length > 0) {
      try {
        ...
        setUploadError("ファイルのアップロードに失敗しました。もう一度お試しください。");
        event.target.value = "";
      }
    }
  }, [addMediaFile]);
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-sm font-extralight">保存済みアップロード</h2>
        <div>
          <Button
            variant="default"
            size="sm"
        ...
        <div className="flex-1 overflow-y-auto">
          {isLoading ? (
            ...
            <div className="h-full flex flex-col font-extralight items-center justify-center py-8 text-muted-foreground text-center">
              ...
              <p className="text-sm text-center">素材ファイルはまだありません</p>
              <p className="text-xs text-center mt-1">
                最初の素材ファイルをアップロードして始めましょう
              </p>
              <Button
                variant="default"
                size="sm"
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {(selectedFile?.type === 'image' ? '画像' : selectedFile?.type === 'video' ? '動画' : selectedFile?.type === 'audio' ? '音声' : selectedFile?.type)} • {formatBytes(selectedFile?.size)}
            </DialogDescription>
          </DialogHeader>
