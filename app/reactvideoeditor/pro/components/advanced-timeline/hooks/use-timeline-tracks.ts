...
export const useTimelineTracks = ({ 
  ...
  const shouldNotifyTracksChangeRef = useRef(false);
  const pendingSelectionRef = useRef<string[] | null>(null);

  const queueTracksChangeNotification = useCallback((nextTracks: TimelineTrack[]) => {
    shouldNotifyTracksChangeRef.current = true;
    return nextTracks;
  }, []);

  // Update internal state when props change
  useEffect(() => {
    shouldNotifyTracksChangeRef.current = false;
  ...
  useEffect(() => {
    if (!shouldNotifyTracksChangeRef.current) {
      return;
    }

    shouldNotifyTracksChangeRef.current = false;

    if (pendingSelectionRef.current && onSelectedItemsChange) {
      onSelectedItemsChange(pendingSelectionRef.current);
      pendingSelectionRef.current = null;
    }

    onTracksChange?.(tracks);
  }, [tracks, onTracksChange, onSelectedItemsChange]);
  ...
  const addNewItem = useCallback((itemData: {
    ...
    try {
      ...
      setTracks(prevTracks => {
        ...
        return queueTracksChangeNotification(updatedTracks);
      });
      
    } catch (error) {
      console.error('Failed to add new item:', error);
    }
  }, [tracks, removeEmptyTracks, queueTracksChangeNotification]);
  ...
  }, [removeEmptyTracks, queueTracksChangeNotification]);
  ...
  const handleInsertTrackAt = useCallback((index: number): string => {
    ...
    setTracks(prev => {
      ...
      return queueTracksChangeNotification(next);
    });
    return newId;
  }, [queueTracksChangeNotification]);
  ...
  const clearSelectionForDeletedTrack = useCallback((trackToDelete: TimelineTrack) => {
    ...
    if (selectedItemsInDeletedTrack.length > 0) {
      ...
      pendingSelectionRef.current = remainingSelectedItems;
    }
  ...
  }, [removeEmptyTracks, clearSelectionForDeletedTrack, queueTracksChangeNotification]);
