...
export function useTimelineHistory(
  ...
  const pendingHistoryApplyRef = useRef<TimelineTrack[] | null>(null);
  ...
  useEffect(() => {
    if (!pendingHistoryApplyRef.current) {
      return;
    }

    const nextTracks = pendingHistoryApplyRef.current;
    pendingHistoryApplyRef.current = null;

    setTracks(nextTracks);
    onTracksChange?.(nextTracks);
  }, [history.present, onTracksChange, setTracks]);
  ...
  const undo = useCallback(() => {
    ...
    setHistory((prev) => {
      ...
      pendingHistoryApplyRef.current = newPresent;
